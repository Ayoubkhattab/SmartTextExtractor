...
def test_paragraph_unit_becomes_an_rtl_right_aligned_paragraph() -> None:
    document = build_docx([[DocumentUnit(kind="paragraph", text="مرحباً بكم")]])
...
def test_heading_unit_becomes_a_heading_2_style_paragraph() -> None:
    document = build_docx([[DocumentUnit(kind="heading", text="عنوان القسم")]])
...
def test_table_unit_becomes_a_real_docx_table_with_matching_cells() -> None:
    unit = DocumentUnit(kind="table", rows=[["A", "B", "C"], ["1", "2"]])  # ragged, like a real messy table
...
def test_multiple_pages_get_a_real_page_break_between_them() -> None:
    document = build_docx(
        [
            [DocumentUnit(kind="paragraph", text="page one")],
            [DocumentUnit(kind="paragraph", text="page two")],
        ]
    )
...
def test_single_page_has_no_page_break() -> None:
    document = build_docx([[DocumentUnit(kind="paragraph", text="only page")]])
...
def test_export_docx_writes_a_file_that_reopens_with_the_same_content(tmp_path: Path) -> None:
    ...
    export_docx([[DocumentUnit(kind="paragraph", text="نص للتحقق")]], path)
